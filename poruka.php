...
if($_SESSION["prijavljen"]=="da"){
	if($_POST["bio"]=="da"){
		...
		$q3="INSERT INTO odgovori (porukaID, korisnikID, poruka)
				VALUES ('$pid', '$korisnikID', '$poruka')";
		mysql_query($q3);
	}
}
$q1="SELECT * FROM poruke WHERE porukaID='$pid'";
$r1=mysql_query($q1);
$nporuka=mysql_num_rows($r1);
if($nporuka==0){
	echo"<p>Nema takve poruke. Ili je obrisana ili je greška u aplikaciji</p>";
}else{
	echo"<table border=\"1\">";
	while($row=mysql_fetch_array($r1)){
		...
		echo"<tr class=\"naslovp\"><td>$naslov</td><td>$vrijeme</td></tr>";
		echo"<tr class=\"tekstp\"><td colspan=\"2\">$poruka</td></tr>";
	}

	$q4="SELECT * FROM odgovori WHERE porukaID='$pid'";
	$r4=mysql_query($q4);
	while($row=mysql_fetch_array($r4)){
		$korisnikID=$row['korisnikID'];
		$poruka=$row['poruka'];
		$vrijeme=$row['vrijeme'];
		echo"<tr class=\"naslovo\"><td>&nbsp;</td><td>$vrijeme</td></tr>";
		echo"<tr class=\"teksto\"><td colspan=\"2\">$poruka</td></tr>";
	}
...
if($_SESSION["prijavljen"]=="da"){
	?>
	<?php echo"<form action=\"poruka.php?pid=$pid\" method=\"post\">"; ?>
  <p>
    <label for="poruka">odgovor</label>
